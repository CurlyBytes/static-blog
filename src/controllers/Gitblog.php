...
class Gitblog extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->blogPath = str_replace("\\", "/", dirname(APPPATH)) . '/public/content/';
	}
	...
	public function exportSite()
	{
		...
		$thfiles = get_dir_file_info("./public/theme/" . $theme, false);
		foreach ($thfiles as $fileName => $file) {
			...
			if ($fileExt != "html") {
				...
				$targetFile = GB_SITE_DIR . substr($serverPath, strpos($serverPath, "/public/theme/" . $theme));
		...
		echo "\ncopy image\n";
		//复制图片文件夹
		$thfiles = get_dir_file_info("./public/content/img/", false);
		foreach ($thfiles as $fileName => $file) {
			$serverPath = $file['server_path'];
			$serverPath = str_replace("\\", "/", $serverPath);

			$targetPath = GB_SITE_DIR . "/public/content/img/";
			$targetFile = GB_SITE_DIR . "/public/content/img/" . $file['name'];
			if (!file_exists($targetPath)) {
				mkdir($targetPath, 0755, true);
			}
			copy($serverPath, $targetFile);
